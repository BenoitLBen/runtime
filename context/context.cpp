...
/* \brief pointer to a function return the worker Id (between 0 and nbWorkers-1)
 * or -1 in a sequential section */
...
void Node::leaveContext() {
  ...
  auto duration =
      std::chrono::duration_cast<ns>(now - current->data.lastEnterTime);
...
void* Node::currentReference() { return (void*)Node::currentNode(); }
...
Node* Node::currentNode() {
  ...
  if (it == currentNodes[index].end()) {
    ...
    if (index != 0) {
      name =
          strdup("Worker #XXX - 0xXXXXXXXXXXXXXXXX");  // Worker ID - enclosing
      assert(name);
      // Avec toyrt, les threads 1 et 2 ne sont pas des workers, ce sont les
      // threads IO & MPI
      if (index == 1)

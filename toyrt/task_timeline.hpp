...
class TaskTimeline {
 private:
  ...
  struct TaskExecution {
    ...
    TaskExecution(const std::string& name, TimePoint _start, TimePoint _stop,
      ...
      start =
          std::chrono::duration_cast<nanos>(_start.time_since_epoch()).count();
      stop =
          std::chrono::duration_cast<nanos>(_stop.time_since_epoch()).count();
    };
  };
  ...
  */
  void addTask(const std::string name, TimePoint start, TimePoint stop,
               std::string extraData) {
